FUNCTION LA_COUNT, var1, dim=dim, mask=mask
;+ 
; NAME: LA_COUNT
; PURPOSE:
;   count number of undefined values inside a lacunar array
;   If mask given extract compute it only inside good values of mask  
; CATEGORY: I-5-c
; CALLING SEQUENCE: 
;   output=LA_COUNT(var1, dim=dim, mask=mask)
; INPUTS: 
;   var1
...
;                0 : return number of every elementary undefined values (default)
;                1 : return vector with number of undefined values of every
;                    "planes". (with as many elements as in the last dimension)
;               -1 : return numebr of undefined values following last dimension
...
;              values.
; OUTPUTS: 
;  output   -- long int : count 
; OPTIONAL OUTPUT PARAMETERS:
;  none 
; EXAMPLE:
;  ICE> print, la_count([[1, 2], [0, 3]])
;         0
;  ICE> print, la_count([[1, 2], [0, 3]], dim=1)
;             0           0
;  ICE> print, la_count([[1, 2], [0, 3]], dim=-1)
;             0           0
; ALGORITHM:
...
;   03-May-1995  written from LA_WEIGHT             FV IAS
;-
...
;------------------------------------------------------------
 ...
 IF N_PARAMS() LT 1 THEN BEGIN
   PRINT, 'CALLING SEQUENCE: output=LA_COUNT(var1, dim=dim)'
...
;------------------------------------------------------------
 ...
 IF szv1(0) EQ 0 THEN BEGIN
    IF var1(0) EQ undef and mask(0) eq 0 THEN output = 1
    GOTO, CLOSING
 ENDIF
 ...
 CASE dim OF
      0 : BEGIN
            good_values = where((var1 eq undef) and (mask eq 0), cpt)
            output = cpt
          END
      1 : BEGIN
            ...
            FOR i = 0l, n_planes-1 DO BEGIN
                ...
                good_values = where((plane EQ undef) and (mask0 eq 0), cpt)
                output(i) = cpt
            ENDFOR
          END
       -1 : BEGIN
            ...
            FOR i = 0l, nele1-1 DO BEGIN
                ...
                index = where((var_array EQ undef) and (mask0 eq 0), cpt)
                output(i) = cpt              
            ENDFOR
          END
