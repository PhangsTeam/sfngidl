FUNCTION LA_DISTA, var1, var2, propdef, undef_ne=undef_ne
;+ 
; NAME: LA_DISTA
; PURPOSE:
;          Compute distance between arrays as sum of absolute difference
;          between elementary values
;          Treat undefined values as equal unless undef_ne set
;          Return proportion of comparable values with propdef
; CATEGORY: I-5-b
; CALLING SEQUENCE: 
;   output=LA_DISTA(var1, var2 [, propdef, /undef_ne])
; INPUTS: 
;   var1, var2 -- array of same dimensions, sizes and types
; OPTIONAL INPUT PARAMETERS:
;   none
; KEYED INPUTS: 
;   undef_ne   -- 0/1 : if set consider undef not equal as undef when computing
;                 propdef
; OUTPUTS: 
;   output     -- float : sum of absolute difference (without undefined values)
; OPTIONAL OUTPUT PARAMETERS: 
;   propdef    -- proportion of comparable values   
; EXAMPLE:
;   ICE> undef = la_undef()
;   ICE> help, la_dista([0,0,undef], [1,2,undef], propdef), propdef
;   <Expression>    FLOAT     =       3.00000
;   PROPDEF         DOUBLE    =        1.0000000
;   ICE> help, la_dista([0,0,undef], [1,2,undef], propdef, /undef_ne), propdef
;   <Expression>    FLOAT     =       3.00000
...
;------------------------------------------------------------
 ...
 IF cpt GT 0 THEN output = total(abs(var1(index) - var2(index))) $
             ELSE output = undef 

 ; proportion of comparable values
